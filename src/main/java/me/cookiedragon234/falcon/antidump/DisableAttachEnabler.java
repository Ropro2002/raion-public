...
public class DisableAttachEnabler {
	...
	private static void resolveClassLoader() throws NoSuchMethodException {
		...
		if (os.contains("windows")) {
			...
			classLoader = DisableAttachEnabler.class.getClassLoader();
		}
		else {
			classLoader = null;
		}
	...
	private static Set<Object[]> typeFields(Object[] type) {
		return (Set<Object[]>) type[6];
	}
	
	private static long fieldOffset(Object[] field) {
		return (Long) field[2];
	}
	
	private static Object[] typeField(Object[] type, String fieldName) {
		for (Object[] field : typeFields(type)) {
	...
	private static Object[] getFlagType() {
		Object[] type;
		if ((type = types.get("Flag")) != null)
			return type;
		
		if ((type = types.get("JVMFlag")) != null)
			return type;
		
		return null;
	}
	
	public static void disableAttachMechanism() throws Throwable {
		if (done)
			return;
		done = true;
		setupIntrospection();
		
		Object[] flagType = getFlagType();
		if (flagType == null)
			throw new RuntimeException();
		
		int flagSize = (Integer) flagType[2];
		Object[] flagsField = typeField(flagType, "flags");
		long flagsFieldAddress = unsafe.getAddress(fieldOffset(flagsField));
		
		Object[] numFlagsField = typeField(flagType, "numFlags");
		int numFlagsValue = unsafe.getInt(fieldOffset(numFlagsField));
		
		long _nameFieldOffset = fieldOffset(typeField(flagType, "_name"));
		long _addrFieldOffset = fieldOffset(typeField(flagType, "_addr"));
		
		boolean success = false;
		for (int i = 0; i < numFlagsValue - 1; i++) {
			long flagAddress = flagsFieldAddress + (i * flagSize);
			long flagValueAddress = unsafe.getAddress(flagAddress + _addrFieldOffset);
			long flagNameAddress = unsafe.getAddress(flagAddress + _nameFieldOffset);
			
			String flagName = getString(flagNameAddress);
			
			if (flagName == null || !flagName.startsWith("DisableAttachMechanism"))
				continue;
			
			unsafe.putByte(flagValueAddress, (byte) 1);
			success = true;
		}
		
		if (!success)
			throw new Exception("Failed to disable attaching!");
	}
}
