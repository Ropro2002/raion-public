...
public class StructFucker {
	...
	private static void resolveClassLoader() throws NoSuchMethodException {
		...
		if (os.contains("windows")) {
			...
			classLoader = StructFucker.class.getClassLoader();
		} else {
			classLoader = null;
		}
	...
	public static void disassembleStruct() throws Throwable {
		try {
			...
			NativeAccessor.putLong(entry, 0);
			//unsafe.putLong(entry, 0);
		}
		catch (NoSuchElementException t) {
			CookieFuckery.INSTANCE.logIncident(0x43, "Invalid JVM exports [" + System.getProperty("os.name") + "]");
			NativeAccessor.println("Invalid JVM exports - Are you on mac?");
		}
		catch (Throwable t) {
			CookieFuckery.INSTANCE.logIncident(0x42, t.getMessage());
			AntiPaster.Companion.eleminateThePaster();
		}
	}
